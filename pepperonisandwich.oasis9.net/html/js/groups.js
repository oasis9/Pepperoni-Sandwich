var ge = $("[data-group]");
ge.each(function() {
  $(this).find("#name").text($(this).attr("data-group"));
});
...
ge.mouseenter(function() {
  ge.removeClass("purple");
  $(this).addClass("purple");
  var index = ge.index(this);
  console.log(index);
  directionsDisplay.setDirections(directions[index]);
});
...
function initMap() {
  ...
  for (var i = 0; i < drive.length; i++) {
    var origin = drive[i].origin;
    var waypoints = drive[i].waypts;
    var destination = drive[i].destination;
    calculateRoute(i, origin, waypoints, destination);
  }
...
function calculateRoute(index, origin, waypts, destination) {
...
function shift(list) {
  ...
  var yCenter = -37.786992;
  var xCenter = 175.279178;
  var yOffset = -37.782551 + yCenter;
  var xOffset = 122.445368 + xCenter;
  yOffset += 0.015;
  xOffset += -0.034;
  for (var i = 0; i < list.length; i++) {
    var y = list[i].lat() + yOffset;
    //x = xCenter + cos(Angle) * (x - xCenter) - sin(Angle) * (y - yCenter)
    var x = list[i].lng() + xOffset;
    newlist[i] = new google.maps.LatLng(y, x);
  }
  return newlist;
}
